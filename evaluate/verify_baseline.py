...
用法
────
  python evaluate/verify_baseline.py --model Qwen/Qwen3-Embedding-8B
  python evaluate/verify_baseline.py --model Qwen/Qwen3-Embedding-8B --model all-MiniLM-L6-v2

输出（evaluate/results/{timestamp}/）
  scores.json   — per-task 分数
...
def run_sts_eval(
    ...
    logger.info(f"  Average ({len(scores)} tasks): {_avg(scores):.4f}")
    return scores
...
# 输出
# ─────────────────────────────────────────────────────────────────────────────
def print_table(all_scores: dict[str, dict[str, float]], model_paths: list[str]) -> None:
    col_w  = 14
    header = f"  {'Task':<22}" + "".join(f"  {_short_name(m):>{col_w}}" for m in model_paths)
    sep    = "  " + "-" * (len(header) - 2)
...
def plot_scores(
    ...
    ax.set_title("MTEB(eng, v1) STS — cosine_spearman", fontsize=11, fontweight="bold")
...
def main() -> None:
    ...
    for model_path in args.models:
        ...
        try:
            all_scores[model_path] = run_sts_eval(model_path, MTEB_STS_TASKS, raw_dir, logger)
        except Exception as exc:
    ...
    try:
        write_report(all_scores, args.models, out_dir / "report.md", timestamp)
        logger.info(f"Report → {out_dir / 'report.md'}")
    except Exception as exc:
        logger.error(f"report.md failed: {exc}")

    try:
        plot_scores(all_scores, args.models, out_dir / "scores.png")
        logger.info(f"Plot   → {out_dir / 'scores.png'}")
    except Exception as exc:
